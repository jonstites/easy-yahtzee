...
| `SimdBatchBuildBackend` | `--features simd` | Outer-loop SIMD: vectorizes the per-state pipeline *across* 8 states with `wide::f32x8`, where each lane is a different state and the matrices/probabilities are broadcast scalar across lanes. Lives in `crates/core/src/linalg/simd_batch.rs`. Pairs `par_chunks(8)` for thread-level parallelism with thread-local scratch via `map_init`; tail (<8 states) falls back to the per-state `SimdBackend`. The two GEMV→masked_max pairs in the per-state pipeline are merged into a single sparse `phase_fused_keeper_round`: walks `KEEPERS_TO_DICE_PROBABILITIES` as CSR (4,368 nonzeros of 116,424, 3.75% dense), computes each row's dot product, and scatter-maxes the result into the output dice in one pass — never materializing the 462-wide `second_keepers` / `first_keepers` intermediates. The masked-max compatibility predicate `mask[d, k] = 1 ⟺ k ⊆ d` is bit-identical to `K2D[k, d] > 0`, so one CSR drives both ops. End-to-end 4.0× over `CpuBuildBackendWith(SimdBackend)` on the validated path (3.13 s → 784 ms); the sparse fused round alone is 26.3× over the dense GEMV+masked_max pair, matching the density ratio's theoretical ceiling. `Error = Infallible`. |
...
- `default = ["simd"]` — the `build` subcommand dispatches through `SimdBatchBuildBackend` (outer-loop SIMD across 8 states + sparse fused keeper round; ~7.4× wall-clock vs `NdarrayBackend`, no API change). The `solve` / `value` / `play` subcommands deserialize the embedded table and don't hit `Scores::new()` regardless of features, so this only affects table regeneration.
