...
| `SimdBatchBuildBackend` | `--features simd` | Outer-loop SIMD: vectorizes the per-state pipeline *across* 8 states with `wide::f32x8`, where each lane is a different state and the matrices/probabilities are broadcast scalar across lanes. Lives in `crates/core/src/linalg/simd_batch.rs`. Pairs `par_chunks(8)` for thread-level parallelism with thread-local scratch via `map_init`; tail (<8 states) falls back to the per-state `SimdBackend`. Six structural wins compound here: (a) the two GEMV→masked_max pairs are merged into a single sparse `phase_fused_keeper_round` (walks K2D as CSR, 4,368 nonzeros of 116,424, 3.75% dense, scatter-maxes directly into `out_dice` without materializing the 462-wide intermediate — ~30× on this stage post-target-cpu); (b) a precomputed `(score + state_scores[child])` table per batch (105 KiB scratch) splits the entry-actions phase into a build phase whose loop nest preserves locality on the random `state_scores[child_idx]` gather, and a branchless SIMD max-reduce phase; (c) `phase_entry_actions_vectorized` runs all 8 lanes' `score_and_child` in one SIMD pass per `(action, dice)` cell — `i32x8` for the bit-pack child_idx computation, `f32x8` for score arithmetic and bonuses, mask-blend for the joker rule; (d) `phase_entry_actions_hoisted` then splits that loop into fast/slow arms by whether `child` depends on dice — for the 6 of 13 actions where it doesn't (3oak / 4oak / FH / SS / LS / chance), `state_scores[child_idx]` hoists out of the d-loop, giving 46% fewer gathers per batch (1.50× on this stage); (e) `phase_entry_actions_per_count` (R5) further reduces the slow-arm 7-action gather count by exploiting that upper actions produce only 6 distinct child states per parent (indexed by `count(face, d)` — 6 different `new_upper_score_remaining` values, all other child fields invariant) and YAHTZEE produces only 2 (indexed by `is_yahtzee(d)`); slow-arm gathers per batch drop from 14,112 to ~304 (40× total batch reduction; 2.46× cache-warm on the entry-actions phase, 1.27× e2e); (f) `process_bucket::<bool>` (R6.5) bucket-sorts states by `!yahtzee_eligible` and dispatches each bucket through a const-generic specialization of `compute_lanes` — when no lane in a batch is yahtzee-eligible (~52–95% of states at heavy levels), the `+100` yahtzee-bonus blend chain DCEs entirely at compile time (-4.4% e2e). End-to-end 25.4× over `CpuBuildBackendWith(SimdBackend)` on the validated path (2.57 s → 101 ms); the entry-actions phase alone is **35× faster** than scalar-per-lane (76.9 µs → 2.17 µs cache-warm). `Error = Infallible`. The BFS reachability filter (`Scores::set_valid_states`) saw two rounds of work: **R3** parallelized it with rayon fold-with-thread-local-bitmap + reduce-OR-merge to avoid serializing the build at 8+ threads (pre-R3: 175 ms sequential and ~50% of wall at 16T; post-R3: 56 ms at 16T); **R7** then hoisted the `LazyLock` derefs out of the BFS hot loop (`State::child` was calling `DICE_AND_ENTRY_SCORES`/`YAHTZEE_DICE` LazyLock checks once per (parent, valid_action, dice_idx) — hundreds of millions of atomic loads per build, 21% of total Ir per cachegrind) and inlined the child-state computation directly on the bit-packed parent idx, collapsing the 252× redundant d-loop for the 6 d-independent actions to a single bit-set. R7 dropped BFS wall to ~29 ms at 16T (-48%) and is -16% e2e on the criterion `Scores::new_with/simd_batch` group. |
...
**Performance numbers, per-level breakdowns, per-stage micro-benches, and the structural-sparsity facts behind the current architecture live in [`crates/core/PERFORMANCE.md`](crates/core/PERFORMANCE.md).** That doc is the canonical home — when build wall-clock or backend ordering changes, update it, not this file. As a one-line summary at time of writing: on Ryzen 7 9800X3D + RTX 5070 Ti (16 threads, `target-cpu=native`), `Scores::new_with` is **7.74 s naive → 101 ms simd_batch → 712 ms cuda**. simd_batch is the fastest backend end-to-end (76.6× over naive, **7.05× faster than CUDA**) after the seven-stage stack listed in the table row above. simd_batch beats CUDA at every level from L=3 upward — CUDA only wins at L=2 and below where its kernel-launch latency amortizes worse and the absolute numbers are sub-ms. Scaling is near-linear on physical cores (96% efficiency at 2T, 91% at 4T, 64% at 8T) but SMT contributes negative — **12 threads is the sweet spot**; 16T regresses slightly (95 → 110 ms wall). Cache miss rate post-R7 is essentially zero (D1 0.1%, LLd 0.0% per cachegrind), so the next big lever isn't cache — it's AVX-512 lane widening of the FMA-throughput-bound `phase_fused_keeper_round` via `wide::f32x16` (Zen 5 has full 512-bit datapaths; wide 1.3.0's `f32x16` lowers directly to `__m512` + `fused_mul_add_m512` when `target_feature="avx512f","fma"` is set).
...
- `default = ["simd"]` — the `build` subcommand dispatches through `SimdBatchBuildBackend` (outer-loop SIMD across 8 states + sparse fused keeper round + precomputed entry-actions table + vectorized `score_and_child` + d-independent gather hoist + per-score-class precompute in slow arm + `!yahtzee_eligible` bucket-sort + parallel BFS + LazyLock-hoisted inline BFS expansion; ~41× wall-clock vs `NdarrayBackend`, no API change). The `solve` / `value` / `play` subcommands deserialize the embedded table and don't hit `Scores::new()` regardless of features, so this only affects table regeneration.
